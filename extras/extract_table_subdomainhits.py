...
for i, domain in enumerate(domains):
    ...
    if os.path.isfile(output_file):
        ...
        for runresult in SearchIO.parse(domtable_file, 'hmmer3-text'):
            ...
            for hsp in sorted(runresult.hsps, key=lambda hsp: -1 * hsp.bitscore):
                if hsp.hit_id not in bitscores:
                    bitscores[hsp.hit_id] = hsp.bitscore
            hit_feature = []
            for clade in clades:
                if clade in bitscores:
                    hit_feature.append(int(bitscores[clade]))
                else:
