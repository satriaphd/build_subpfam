...
def main():
    ...
    for i, fasta_path in enumerate(fasta_files):
        ...
        output_path = "{}.sub.hmm".format(path.join(output_folder, path.splitext(path.basename(fasta_path))[0]))
        print("Building clade HMM: {} ({}/{})".format(output_path, i+1, len(fasta_files)))
        build_hmm(fasta_path, clade_txt_path, output_path)


def build_hmm(fasta_path, clade_txt_path, ouput_hmm_path):
    ...
    with TemporaryDirectory() as temp_dir:
        for cl in clades:
            ...
            if subprocess.check_call(command, shell=True) == 0:
                continue
        with open(ouput_hmm_path, "w") as hm:
            for cl in clades:
                hmm_name = "{}_c{}".format(pfam_name, cl)
