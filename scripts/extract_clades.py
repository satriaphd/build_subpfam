...
def main():
    ...
    tree_files = glob.glob(path.join(trees_folder, "*.tree"))
    commands = []
    for i, tree_path in enumerate(tree_files):
        ...
        if midroot_folder is not None:
            midroot_path = "{}.tree".format(path.join(midroot_folder, path.splitext(path.basename(tree_path))[0]))
        split_tree_smart(tree_path, output_path, midroot_path)
    

def split_tree_smart(newick_file, out_file, processed_newick_out = None):
    ...
    # write clades
    clades = [[node.name for node in branch] for branch in sorted(branches, key=lambda nodes: -1 * len(nodes))]
    with open(out_file, "w") as th:
